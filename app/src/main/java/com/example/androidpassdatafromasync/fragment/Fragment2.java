...
public class Fragment2 extends Fragment implements NetworkResponseListener {
    ...
    public void SuccessData(String data) {
        ...
        try {
            ...
            String photos=jsonObject1.getString("photos");
            JSONObject jsonObject2= new JSONObject(photos);
            String photo =jsonObject2.getString("photo");
            List<ImageModel> imageModels=new ArrayList<>();
            JSONArray jsonArray = new JSONArray(photo);
            for (int i=0;i<jsonArray.length();i++){
                imageModels.add(new ImageModel(jsonArray.getJSONObject(i).getString("title"),jsonArray.getJSONObject(i).getString("url_s")));
            }
